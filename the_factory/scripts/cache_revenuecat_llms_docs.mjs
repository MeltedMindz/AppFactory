...
const OUT_DIR = join(FACTORY_ROOT, 'vendor', 'revenuecat-docs');
const OUT_FILE = join(OUT_DIR, 'llms.txt');
const META_FILE = join(OUT_DIR, 'meta.json');

// Configuration
const SOURCE_URL = 'https://www.revenuecat.com/docs/llms.txt';
...
async function main() {
  try {
    console.log('🚀 Caching RevenueCat LLM documentation...');
    ...
    console.log('🎉 RevenueCat documentation cached successfully!');
    console.log(`📈 Size: ${content.length} bytes`);
    console.log(`🔑 SHA256: ${sha256}`);
    console.log(`📅 Timestamp: ${metadata.downloadedAt}`);
    
  } catch (error) {
    console.error('❌ REVENUECAT DOCS CACHE FAILED:');
